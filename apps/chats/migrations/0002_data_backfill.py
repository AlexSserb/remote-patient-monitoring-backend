...
def _get_or_create_direct_chat(Chat: object, user_a: object, user_b: object, patient: object) -> None:
    """Создаёт чат между двумя пользователями в контексте пациента, если он ещё не существует."""
...
def create_chats(apps: object, schema_editor: object) -> None:
    ...
    for dp in DoctorPatient.objects.select_related("doctor", "patient"):
        _get_or_create_direct_chat(Chat, dp.doctor, dp.patient, dp.patient)
        for cp in CaregiverPatient.objects.filter(patient=dp.patient).select_related("caregiver"):
            _get_or_create_direct_chat(Chat, dp.doctor, cp.caregiver, dp.patient)
    ...
    for cp in CaregiverPatient.objects.select_related("caregiver", "patient"):
        _get_or_create_direct_chat(Chat, cp.caregiver, cp.patient, cp.patient)
        for dp in DoctorPatient.objects.filter(patient=cp.patient).select_related("doctor"):
            _get_or_create_direct_chat(Chat, cp.caregiver, dp.doctor, cp.patient)
        for other_cp in (
            CaregiverPatient.objects.filter(patient=cp.patient)
            .exclude(caregiver=cp.caregiver)
            .select_related("caregiver")
        ):
            _get_or_create_direct_chat(Chat, cp.caregiver, other_cp.caregiver, cp.patient)
